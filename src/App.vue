...
onMounted(() => {
  ...
  window.addEventListener("load", function () {
    let orientation = getOrientationNum();
...
const getOrientationNum = () => {
  let orientation = 0
  //@ts-ignore
  if (!isNaN(window.orientation)) {
    orientation = window.orientation
  } else if (window.screen.orientation && !isNaN(window.screen.orientation.angle)) {
...
<template>
  ...
  <audio ref="musicRef" src="" :style="{ position: 'fixed', zIndex: -5, opacity: 0 }">
    您的浏览器不支持 audio 标签。
  </audio>
</template>
